...
class Llama:
    ...
    def build(
        ...
        assert len(checkpoints) > 0, f"no checkpoint files found in {llama_ckpt_dir}"
        assert model_parallel_size == len(
            checkpoints
        ), f"Loading a checkpoint for MP={len(checkpoints)} but world size is {model_parallel_size}"
        llama_ckpt_path = checkpoints[get_model_parallel_rank()]
        llama_checkpoint = torch.load(llama_ckpt_path, map_location="cpu")
        ...
        # Load dynamic Router weights into Transformer.
